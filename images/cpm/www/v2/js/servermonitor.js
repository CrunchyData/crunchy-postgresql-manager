(function() {
	...
	function loadSeries(points) {
        ...
        $scope.handleRefresh = function(interval) {
            ...
            $http.get($cookieStore.get('AdminURL') + '/mon/server/' + 'cpu' + '.' + $scope.server.ID + '.' + interval + '.' + token).
            success(function(data, status, headers, config) {
		    if (data.length > 0) {
	...
	memyAxis.render();
        ...
        $scope.memhandleRefresh = function(interval) {
            var token = $cookieStore.get('cpm_token');
            $http.get($cookieStore.get('AdminURL') + '/mon/server/' + 'mem' + '.' + $scope.server.ID + '.' + interval + '.' + token).
            success(function(data, status, headers, config) {
		    if (data.length > 0) {
